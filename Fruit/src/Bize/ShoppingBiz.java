...
public class ShoppingBiz {
	...
	public void add(ShoppingAddress shoppingAddress) {
		...
		DBHelper.insert(sql, shoppingAddress.getUid(), shoppingAddress.getSname(), shoppingAddress.getStel(),
				shoppingAddress.getSprovince(), shoppingAddress.getScity(), shoppingAddress.getScounty(),
				shoppingAddress.getSstreet(), shoppingAddress.getSpostcode(), shoppingAddress.getSinput());
	}
	...
	public void zhangdan(checks check) {
		...
		DBHelper.insert(sql1, check.getUid(), check.getCdate(), check.getSendaddr(), check.getSenddata(),
				check.getFapiao(), check.getSleave(), check.getZongji(), "未发货");
	}
