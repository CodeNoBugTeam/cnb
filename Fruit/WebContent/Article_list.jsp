...
<input type="hidden" name="op" value="messa">
 分类：<input name="theme" type="text" value="${param.theme }"/>
 标题：<input name="mtitle" type="text" value="${param.mtitle }"/>
 状态：<input name="mstate" type="text" value="${param.mstate }"/>
<input  class="btn button_btn bg-deep-blue " href="message.s?op=query" type="submit" value="搜索"/>
 <span>数量:3433件</span>
 </form>
    ...
    <thead>
        ...
        <tbody>
      	 <c:forEach items= "${messageList }" var="u">
	         ...
	         <a rel="${u.mid }" title="发布" href="message.s?op=fabu&messageId" class="btn btn-xs btn-status">发布</a> 
	         <a rel="${u.mid }" title="编辑" href="add_Article.jsp"  class="btn btn-xs btn-info" >编辑</a> 
